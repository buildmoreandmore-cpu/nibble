  ...
  if (step === 0) {
    return (
      <div className="min-h-screen bg-white relative overflow-hidden">
        ...
        {/* Mobile version - larger logo on right side */}
        <div className="md:hidden absolute top-24 -right-8 pointer-events-none">
          <img src="/logo.png" alt="" className="w-[200px] h-auto opacity-[0.20] object-contain" />
        </div>
          ...
          <div className="text-left">
            ...
            <div className="mt-12 md:hidden bg-gradient-to-br from-emerald-50 to-teal-50 rounded-2xl p-6 border border-emerald-100">
              ...
              <div className="flex flex-wrap gap-1.5 mb-4">
                {['chicken', 'rice', 'cheese'].map((item, i) => (
                  <span key={i} className="px-2 py-1 bg-white border border-emerald-200 rounded-full text-xs font-medium text-emerald-700">
                    {item}
                  </span>
                ))}
                <span className="px-2 py-1 bg-emerald-500 text-white rounded-full text-xs font-bold">
                  → ideas
                </span>
              </div>
              <div className="space-y-1.5">
                {['Uses what you have', 'Respects allergies', 'Age-appropriate'].map((item, i) => (
        ...
        {/* NEW: Fridge Ideas Feature Highlight */}
        <section className="max-w-7xl mx-auto px-4 sm:px-6 pb-16 relative z-10">
          <div className="bg-gradient-to-br from-emerald-50 to-teal-50 rounded-3xl p-6 sm:p-8 lg:p-12 border border-emerald-100 overflow-hidden relative">
            ...
            <div className="relative md:flex md:items-center md:gap-12">
              ...
              <div className="flex-1 mb-8 md:mb-0">
                ...
                <p className="text-slate-600 mb-6 max-w-lg text-sm sm:text-base">
                  Got random ingredients but no idea what to make? Just tell us what you have —
                  we'll suggest <strong>personalized meal ideas</strong> that work for your child's age and preferences.
                </p>
                <div className="flex flex-wrap gap-2 mb-6">
                  {['chicken', 'broccoli', 'rice', 'cheese'].map((item, i) => (
                    <span key={i} className="px-3 py-1.5 bg-white border border-emerald-200 rounded-full text-sm font-medium text-emerald-700">
                      {item}
                    </span>
                  ))}
                  <span className="px-3 py-1.5 bg-emerald-500 text-white rounded-full text-sm font-bold">
                    = 4 meal ideas
                  </span>
                </div>
                ...
                <div className="bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden">
                  ...
                  <div className="p-4 space-y-3">
                    <div className="bg-gray-50 rounded-xl p-3 border border-gray-100">
                      <p className="text-xs text-slate-400 mb-1">Your ingredients:</p>
                      <p className="text-sm font-medium text-slate-700">chicken, broccoli, rice, cheese</p>
                    </div>
                    {[
                      { title: 'Cheesy Chicken Rice Bowl', tags: ['chicken', 'rice', 'cheese'] },
                      { title: 'Broccoli Cheese Bites', tags: ['broccoli', 'cheese'] },
                    ].map((idea, i) => (
                      <div key={i} className="p-3 rounded-xl border border-emerald-100 bg-emerald-50/50">
                        <p className="font-bold text-brand-dark text-sm mb-2">{idea.title}</p>
                        <div className="flex flex-wrap gap-1">
                          {idea.tags.map((tag, j) => (
                            <span key={j} className="text-[10px] px-2 py-0.5 bg-emerald-100 text-emerald-700 rounded-full">
                              {tag}
                            </span>
                          ))}
                        </div>
                    ...
                    <p className="text-[10px] text-slate-400 text-center">+ 2 more ideas</p>
                  </div>
                </div>
              </div>
            </div>
